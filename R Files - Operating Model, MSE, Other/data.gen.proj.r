# Size-structured MSE 
# AM data generation updated file
# M. Damiano
# Last updated: 8-23-2023

# Call OM functions needed to update data files
# Projected population dynamics functions are in the mse.proj and Projection files
...
sels <- cal_sels(sel_pars_v,sel_switch,dome_pars,size_bm)
...
catch.proj.cv = array(cv, c(n_proj, n_fleets, n_s))
catch.proj.ess = array(ess, c(n_proj, n_fleets, n_s))

# create arrays of survey cv and ess by year, survey and season
survey.proj.cv = array(cv, c(n_proj, n_s, n_survey))
survey.proj.ess = array(ess, c(n_proj, n_s, n_survey))

# Update catch and survey data
catch.data.proj <- catch.data.gen(Year.proj.start,Year.proj.end,catch.obj = catch.proj.obj,catch.proj.cv,catch.proj.ess,add.error)
catch.data <- dfOrder(rbind(catch.data,catch.data.proj),c(3,1))

survey_periods_proj = matrix(NA, ncol = 2, nrow = n_survey)
for(n in 1:n_survey)
{
  survey_periods_proj[n,] = c(Year.proj.start, Year.proj.end)
}
survey.data.proj <- survey.data.gen(survey_periods_proj,survey_months,n_survey,survey.obj=survey.proj.obj,survey.proj.cv,survey.proj.ess,add.error)
survey.data <- rbind(survey.data, survey.data.proj)
...
cat("#Fleet selectivity parameters","\n")
for (i in 1:8){ #must match total number of sel pars
