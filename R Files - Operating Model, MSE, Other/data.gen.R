# Size-structured MSE 
# AM data generation file
# M. Damiano
# Last updated: 8-23-2023

# Call OM functions to produce values for data gen
# plots underneath each function are there for quick and dirty checks, as needed
...
catch.data <- catch.data.gen(year_start,year_end,catch.obj,catch.cv,catch.ess,add.error)
survey.ess = array(100, c(survey_periods[,2]-survey_periods[,1]+1,n_survey,n_s))
survey.data <- survey.data.gen(survey_periods,survey_months,n_survey,survey.obj,survey.cv,survey.ess,add.error)

# These write all .dat files for running the OM
...
cat(c(0),"\n")
cat("#CohortTrackingBeginYear (track a cohort) ","\n")
cat(c(year_start),"\n")
cat("# Check Number","\n")
cat(c(999),"\n")
sink()
